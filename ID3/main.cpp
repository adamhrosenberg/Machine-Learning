...
public:
	 ...
	 */
	void populateAttributeTable(vector<string> names, vector<vector<bool> >* attributetable){
		for(int i = 0; i < names.size(); i++){
...
//		cout << "# of total = " << labels.size() << endl;
		...
		label_entropy = -positivep * log2(positivep) - negativep * log2(negativep);
	...
	void computeEntropy(vector<bool> feature){
